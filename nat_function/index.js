...
var username = 'panw'
var password = 'sompassword'
//TODO: Username and password are hard coded here because the
// NAT function doesn't run as expected when run in an Azure function.
// Hence was not setup to be read in as an environment variable. 
// What needs to be done further is to expose this username and password
// as environment variables (i.e app settings on the azure function) and
// then read in by accessing the environment variables from the function
...
//TODO: Please see note above regarding usage of environment variables.
// The resource group name needs to be read in as an environment 
// variable as well. 

resource_gp_name = 'vvend2endv2'
// see documentation for the ssh2 npm package for a list of all options  
// TODO: see TODO (1) to read in the username and password as environment 
// variables. 
...
// This is the service principal information 
// This is a one time setup that needs to be done
// out of band and used here. 
...
function execute_ssh_command_with_output(tmp_config, remote_port) {
    ...
    s_exec(tmp_config, command, function (error, response) {
        if (error) {
            console.info('Unable to communicate with instance mapped onto port ' + remote_port)
            console.info('The reason for failure is most likely because there is no instance mapped on that port. Error' + error)
        } else {
            console.log('ssh command response: ' + response);
...
function process_lb(data, resourceGroupName) { 
   ...
   if (data.publicIPAddress == undefined) {
    ...
    console.info('Print out the internal load balancer details')
    console.info(internal_load_balancer)
   } else {
...
function identify_nat_ports(_name) {
    ...
    nw_client.loadBalancers.get(resource_gp_name, _name, expand='inboundNatRules', function(err, result) {
                    if (err) {
                        console.error('Error retrieving nat rules' + err)
                    } else {
...
function handle_pub_ip_response(err, result) {
    ...
    if (err) {
       ...
       console.info('Public IP Response ' + result)
...
module.exports = function (context, myTimer) {
    context.log('Timer Triggered function to handle programming NAT rules on FW');
    msRestAzure.loginWithServicePrincipalSecret(clientId, secret, domain, function(err, credentials) {
        if(err) {
                console.log(err);
        } else {
            //console.info(credentials)
            resource_gp_name = GetEnvironmentVariable('PAN_RESOURCE_GROUP_NAME')
