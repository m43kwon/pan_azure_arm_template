...
function check_nat_exists() {
    ...
    connection_options['username'] = admin_username
    connection_options['password'] = admin_password
...
function execute_remote_ssh_command(e_config, cmds_list) {
    rexec(hosts, cmds_list, e_config, function(err){
        if (err) {
            console.log(err);
            return 1;
        } else {
...
if (process.argv.length < 5) {
    console.error('Usage: ' + __filename + ' <username> <password> <resource group name>')
    process.exit(-1)
} else {
    console.log(process.argv[0], process.argv[1], process.argv[2], process.argv[3])
    resource_gp_name = process.argv[4]
    admin_username = process.argv[2]
